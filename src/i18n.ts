...
const TERMS: [string, string][] = [
  ...
  ['필렛과 간섭 — 내부 이음판 폭/게이지 조정 필요', 'interferes with fillet — adjust inner-plate width/gauge'],
  ['임팩트렌치 진입 간섭 우려', 'impact-wrench access may interfere'],
  ['외부 이음판만 사용 → 외부 이음판이 전 축력을 부담한다.', 'Outer plate only → it develops the full axial force.'],
  ['내·외부 이음판 동일 두께(합성 순단면 기준)', 'Inner = outer plate thickness (combined net section)'],
  ['상·하 플랜지 중심거리 (H−tf)', 'centroidal distance of top/bottom flanges (H−tf)'],
  ['H형강 전체 춤 (외부 이음판만 사용)', 'full H-section depth (outer plate only)'],
  // 3D 범례·힌트
  ...
  ['내부 이음판 안쪽 ↔ 필렛 끝단', 'Inner-plate face ↔ fillet toe'],
  ...
  // 2D 뷰 캡션
  ['플랜지 평면도 (외부 이음판)', 'Flange plan (outer plate)'],
  ...
  ['플랜지 이음판 폭·두께', 'Flange Splice-Plate Width·Thickness'],
  ['플랜지 볼트 설계강도·배열', 'Flange Bolt Design Strength·Layout'],
  ['플랜지 이음판 길이', 'Flange Splice-Plate Length'],
  ['웨브 소요전단강도', 'Web Required Shear Strength'],
  ['웨브 이음 소요력', 'Web Splice Demand'],
  ['웨브 볼트 · 이음판', 'Web Bolt · Splice Plate'],
  // 라벨(계산서)
  ['설계지압강도 φRn₁/φRn₂ (연단/중간)', 'Design Bearing Strength φRn₁/φRn₂ (edge/interior)'],
  ['웨브 이음판 (두께×춤×너비)', 'Web Splice Plate (t×depth×width)'],
  ['이음판 두께(내·외 동일)', 'Splice-Plate Thickness (inner = outer)'],
  ['접합부 소요휨강도', 'Connection Required Flexural Strength'],
  ['이음판 소요단면적', 'Splice-Plate Required Area'],
  ...
  ['내부 이음판~웨브볼트', 'inner PL~web bolt'],
  ['내부 이음판끝~필렛', 'inner PL edge~fillet'],
  ['외부 이음판 L=', 'outer PL L='], ['웨브 H=', 'web H='], ['웨브 L=', 'web L='],
  ['웨브판t', 'web PL t'],
  ['외부 이음판폭', 'outer PL w'], ['외부 이음판t', 'outer PL t'],
  ['내부 이음판폭', 'inner PL w'], ['내부 이음판t', 'inner PL t'],
  ['내부 이음판 두께', 'Inner Splice-Plate Thickness'], ['외부 이음판 두께', 'Outer Splice-Plate Thickness'],
  ['플랜지 이음판', 'Flange plate'], ['웨브 이음판', 'Web plate'],
  ['이음판 길이', 'Splice-Plate Length'],
  ['연단(직각)', 'edge (transv.)'],
