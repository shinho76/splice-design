      ...
      <div className="report-card doc aisc" onClick={e => e.stopPropagation()}>
        ...
        <section className="doc-sec">
          ...
          <table className="result-table2"><tbody>
            ...
            <tr><th>{L('보정 후', 'Corrected')}</th><td><b>{L('외첨판', 'Outer')} PL-{r.flange.outerPlate?.t}×{r.flange.outerPlate?.w} · {L('내첨판', 'Inner')} PL-{r.flange.innerPlate?.t}×{r.flange.innerPlate?.w}×2 · {L('볼트', 'Bolt')} {r.flange.bolt.m}×{Math.round(r.flange.bolt.n)}-M{r.boltDia}</b> {ac.memberLimited && <span className="ag-ng">· {L('부재 단면 한계(단면 상향 필요)', 'member section limited')}</span>}</td></tr>
