      ...
      <div className="report-card doc aisc" onClick={e => e.stopPropagation()}>
        ...
        <section className="doc-sec">
          ...
          <table className="result-table2"><tbody>
            <tr><th>{L('표준 배치', 'Standard layout')}</th><td>{L('외부 이음판', 'Outer')} PL-{result.flange.outerPlate?.t}×{result.flange.outerPlate?.w} · {L('내부 이음판', 'Inner')} PL-{result.flange.innerPlate?.t}×{result.flange.innerPlate?.w}×2 · {L('볼트', 'Bolt')} {result.flange.bolt.m}×{Math.round(result.flange.bolt.n)}-M{result.boltDia}</td></tr>
