...
const INTRO: Record<string, [string, string]> = {
  ...
  '플랜지 이음판 폭·두께': ['소요축력을 이음판 강종의 설계항복강도로 나눠 소요단면적을 구하고, 표준 폭에서 두께를 정한다(내·외 분담).',
  ...
  '플랜지 이음판 길이': ['볼트 배열·피치·연단·이음갭으로 이음판 길이를 정한다.',
  ...
  '웨브 볼트 · 이음판': ['웨브 볼트 1개 설계강도로 필요 개수를 정하고, 춤·피치 제약 하에서 배열과 이음판 치수를 정한다.',
      ...
      <div className="report-card doc narr" onClick={e => e.stopPropagation()}>
        ...
        <section className="doc-sec">
          ...
          <table className="result-table2"><tbody>
            ...
            <tr><th>{L('플랜지 외부 이음판', 'Flange outer plate')}</th><td>{plate(result.flange.outerPlate)} ×2</td></tr>
            <tr><th>{L('플랜지 내부 이음판', 'Flange inner plate')}</th><td>{result.flange.innerPlate ? `${plate(result.flange.innerPlate)} ×4` : '—'}</td></tr>
            ...
            <tr><th>{L('웨브 이음판', 'Web plate')}</th><td>{plate(result.web.webPlate)} ×2</td></tr>
          </tbody></table>
        </section>
