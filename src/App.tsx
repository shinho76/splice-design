...
import { toDXFShear, toDXFShearAll } from './engine/shear/dxfShear.ts';
...
export default function App() {
  ...
  // Custom 볼트직경 해석 : 행별 독립 지정(다른 행에 전파되지 않음)
  const diaAt = useCallback((i: number): number | undefined => {
    ...
    if (boltMode === 'Custom' && allowCustom) return boltOv[i] ?? stdDia;   // 지정 없는 행: K=S규칙값 / A=엔진기본(undefined)
  ...
  // GS(GIRDER SPLICE) 전용: 강도비 '지정' — 행별 독립 강도비(%) 오버라이드(다른 행에 전파되지 않음)
  ...
  const alphaAt = useCallback((i: number): number | undefined => {
    ...
    return alphaOv[i] ?? Math.round(cond.strengthRatio * 100);
  ...
  const exportShearDetailAllDXF = () =>
    downloadFile('DETAIL_전체부재_SC.dxf', toDXFShearAll(scRows(), cond), 'application/dxf');
      ...
      <aside className="rail">
        ...
        <div className="rmenu">
          ...
          <div className="rmenu-pop" role="menu">
            ...
            <button type="button" className="rmenu-item" role="menuitem" onClick={exportShearDetailAllDXF} title={L('전체 단면 상세도 DXF(검토용) — 호칭 춤 시리즈별 그리드', 'All-section detail DXF (for review) — grid grouped by nominal depth series')}>🗂 {L('DETAIL DXF(검토용)', 'DETAIL DXF (review)')}</button>
